...
- 수집 구간은 `2026-01-01 08:00 ~ 2026-01-18 16:35`(5분 간격)이다.

### 검증 지표

> **스포일러 주의** — 아래 표에는 교재 6장에서 독자가 직접 규명할 원인이 드러나 있다. 학습자로서 저장소를 방문했다면 6장을 마친 뒤에 읽기를 권한다.
...
두 가지 불변 조건도 함께 확인한다. 불량 판정(`Pass_Fail == 0`)인 행의 에러 코드는 모두 `ERR-T101`이어야 하고, `ERR-G202`가 붙은 173행은 모두 양품이어야 한다. 후자는 11장 예지 보전의 선행 신호로 쓰이므로 불량과 섞이면 안 된다.
...
- 1~5장 코드에서 `ETCHER_B`를 원인으로 서술하거나 온도 임계선(85℃)을 상수로 박아 두지 않는다.
- 장비별 불량률 비교, 온도 구간별 불량률 산출은 6장 코드부터 등장시킨다.
- 1~5장에서 노출 가능한 범위는 전체 불량률 16.08%와 로트별 추이까지다.
